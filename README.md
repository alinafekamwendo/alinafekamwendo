# Hi, I’m Alinafe C. Kamwendo 

 **Fullstack Software Developer|ICT Facilitator | Systems & Data Enthusiast**  
 Lilongwe, Malawi  
 Portfolio: https://alinafeckamwendo.vercel.app/

---

## About Me
I am a passionate Computer Scientist and educator with hands-on experience in **web systems development, Android applications, data analysis, and enterprise IT systems**. I have worked across education, government,research and community programs.

I enjoy building secure, scalable systems and continuously improving my technical and professional skills.

---

##  Technical Skills
- **Programming & Development:**  
  JavaScript, React, Next.js,NestJs, Node.js, Express, Java

- **Databases & Systems:**  
  PostgreSQL, MySQL, MongoDB, Prisma, Sequelize

- **Data & Analytics:**  
  DHIS2, Kobo, Excel, SPSS, Data Visualization

- **IT & Infrastructure:**  
  Linux & Windows Server Administration, Active Directory,networking, Hardware & Software installation and maintenance

- **Other Areas:**  
  Cybersecurity fundamentals, Disaster Recovery Planning, Graphics Design, computer networking

---

##  Experience Highlights
- **Computer Science Tutor** – University of Malawi 
  Supporting first-year students in programming and problem-solving.

- **ICT Teacher** – Secondary School Level  
  Delivering engaging Computer Studies and Mathematics lessons.

- **Data Clerk** – Ministry of Health, Malawi  
  Health data collection, validation, reporting, and visualization using DHIS2.

- **Systems Analyst Intern** – Dedza District Council  
  Database administration, technical support, and systems management.

- **Google Android Developer Mentor (Volunteer)** – Andela Learning Community  
  Mentoring developers in the Google African Developer Training Program.

---

##  What I’m Currently Focused On
- Full-stack web applications (Next.js, APIs, PostgreSQL)
- Secure authentication & system design
- Educational technology solutions
- Cybersecurity and system reliability

---

##  Featured Work
Check out my live projects on my portifolio
They showcase:
- Web APIs & backend systems  
- Frontend applications  
- Learning and teaching projects  

---

##  Let’s Connect
-  Portfolio: https://alinafeckamwendo.vercel.app/
- Email: alinafekamwendo11@gmail.com / kamwendoalina@gmail.com
  
